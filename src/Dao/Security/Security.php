...
class Security extends \Dao\Table
{
    ...
    static public function newUsuario($email, $username, $password)
    {
        ...
        if (!\Utilities\Validators::IsValidName($username)) {
            throw new Exception("El nombre de usuario debe tener al menos 3 caracteres y no contener símbolos especiales.");
        }
    ...
    //verificar permisos del historial de compras
    public static function getRolesByUser($userId)
    {
        $sql = "SELECT rolescod FROM roles_usuarios WHERE usercod = :userId";
        $params = ["userId" => $userId];
        $results = self::obtenerRegistros($sql, $params);
        $roles = array_map(fn($row) => $row["rolescod"], $results);
        return $roles;
    }
